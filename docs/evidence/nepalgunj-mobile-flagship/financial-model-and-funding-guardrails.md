# Nepalgunj Flagship Financial Model and Funding Guardrails

**Document ID:** EVD-NPJ-FLAGSHIP-005
**Version:** 0.4.0
**Status:** Planning model — bank, distributor, rent, payroll and professional inputs pending
**Accountable owner:** Founder until Finance Owner is appointed

**Related research/model:** [Integrated market size, assortment and flagship defensibility](../../research/integrated-market-size-assortment-and-flagship-defensibility.md) and [KAMLESH Flagship Commercial Evidence Model](./KAMLESH-Flagship-Commercial-Evidence-Model.xlsx)

## Funding concept

- Proposed total bank-facility ceiling: NPR 1 crore
- Proposed distributor bank-guarantee ceiling: NPR 60 lakh
- Proposed cash-credit/working-capital and/or term-loan ceiling: NPR 40 lakh; exact bank-approved product split remains open
- Recommended initial total utilisation: NPR 60–70 lakh
- Recommended initial BG-supported anchor/halo handset exposure: NPR 30–35 lakh
- Recommended undrawn/liquidity protection: NPR 30–40 lakh

The founder currently intends the BG side for approved authorised anchor/halo distributors such as Apple, Samsung, Nothing or equivalent relationships, and the NPR 40 lakh facility/equity mix for eligible short-cycle inventory and operations plus separately structured longer-lived fit-out/startup needs. The bank and each distributor must confirm product, permitted use and eligibility; naming a brand here does not establish supply, credit or BG acceptance.

A bank guarantee is a contingent liability, not free stock capital. Invocation creates an obligation to the bank. Commission, collateral/margin, expiry, renewal and claim procedures must be modelled separately.

## Controlled opening allocation hypotheses

The envelopes below are planning scenarios, not approved allocations or authority to draw debt, issue a BG or place stock orders. Rebuild them from the SKU-level gate after local demand, exact supplier economics and downside liquidity are evidenced.

| Use | Facility source | Opening guideline |
|---|---:|---:|
| Authorised anchor/halo handset exposure | BG ceiling NPR 60 lakh | NPR 30–35 lakh initially |
| Five-shutter shell, glazing, ceiling, floor, electrical, AC, fixtures and signage | Bank-approved term-loan/equity side; do not assume cash-credit eligibility | NPR 9–12 lakh target |
| POS, IMEI/inventory system, CCTV, alarm, safe, networking and backup | Bank-approved term-loan/equity side; do not assume cash-credit eligibility | NPR 3–4 lakh target |
| Premium original accessories and wearables | Bank-approved short-cycle working-capital side | NPR 4–5 lakh target |
| Authorised smaller-brand handset/laptop inventory | Bank-approved short-cycle working-capital side | NPR 6–8 lakh within the NPR 35–42 lakh total device envelope |
| K&S Care tools and initial consumables | Bank-approved facility/equity according to permitted use | NPR 1–2 lakh initially |
| Lease, registration, insurance and professional fees | Equity or bank-approved facility according to permitted use | NPR 2–3 lakh; terms TBD |
| Pre-launch, recruitment, uniform and training | Equity or bank-approved facility according to permitted use | NPR 1.5–2.5 lakh |
| Undrawn facility/liquidity capacity retained at opening | NPR 40 lakh combined ceiling scenario | Minimum NPR 5–10 lakh; founder/bank approval open |
| **Combined ceiling** | **NPR 1 crore** | **Initial exposure target NPR 60–70 lakh** |

The funded-side target envelopes total approximately NPR 26.5–36.5 lakh before retained capacity, but the opening draw target is NPR 30–35 lakh. Do not force every category to the top of its range. If essential fit-out, security, lease or opening inventory quotations would consume the full NPR 40 lakh, reduce assortment and decorative scope before reducing the liquidity floor.

## Finance-to-asset matching

- Term loan funds durable fit-out, electrical work, AC, power backup, security and service equipment.
- Cash-credit/approved working-capital funds only eligible short-cycle working capital and timing gaps. Durable fit-out should use equity or a bank-permitted term-loan structure matched to asset life; obtain written permitted-use confirmation.
- BG supports authorised distributor purchases under documented credit and settlement terms.
- Slow or premium handset inventory is not justified merely because the facility is available.

## Model inputs to replace

| Input | Provisional assumption | Required evidence |
|---|---:|---|
| Average handset price | NPR 29,000–30,500 | Actual assortment and sell-through |
| Handset gross margin | 5.5% base | Written distributor schedule |
| Accessory gross margin | 35–45% | Supplier quotation and test sales |
| Service contribution margin | 50–70% | Technician/parts model |
| Rent | Not approved | Five quotations and selected lease |
| Five-shutter usable area | Approximately 60 ft × 12 ft / 720 sq ft planning value | Measured internal survey confirming clear dimensions and orientation |
| Fit-out | NPR 9–12 lakh target envelope | Scope-aligned local quotations and variation allowance |
| Payroll | Not approved | Approved roles, salary and statutory cost |
| Interest/BG cost | Not approved | Bank term sheet and sensitivity |
| Stock ageing | Under 45 days target | Live inventory reports |

## Break-even planning range

At an early blended gross margin near 9%:

| Fully loaded monthly cost | Approximate revenue break-even | Approximate monthly phones, assuming 90% phone revenue at NPR 29,000 |
|---:|---:|---:|
| NPR 1.5 lakh | NPR 16.8 lakh | 52 |
| NPR 1.8 lakh | NPR 20.1 lakh | 62 |
| NPR 2.2 lakh | NPR 24.6 lakh | 76 |
| NPR 2.6 lakh | NPR 29.1 lakh | 90 |

These are planning calculations, not forecasts. The final model must include interest, principal repayment, BG commission, tax, discounts, warranty exposure, shrinkage and depreciation.

## First-year commercial target hypotheses

These targets are model inputs to test against at least two independent local demand methods, four weeks of qualified-visit/conversion evidence and realized contribution. They are not market findings, forecasts or approved market-share commitments.

| Period | Handset units | Total sales target including accessories/services |
|---|---:|---:|
| Month 1 | 45 | NPR 15–18 lakh |
| Month 2 | 55 | NPR 18–21 lakh |
| Month 3 | 70 | NPR 23–27 lakh |
| Quarter 2 | 255 total | NPR 80–90 lakh |
| Quarter 3 | 300 total | NPR 100–110 lakh |
| Quarter 4 | 345 total | NPR 115–130 lakh |
| **Year 1 base target** | **Approximately 1,070 phones** | **Approximately NPR 3.7–3.9 crore** |

Targets do not authorise debt drawdown. BG or working-capital expansion requires gross profit, cash collection, stock ageing, due-date cover and downside liquidity evidence.

## Minimum controls

- Debt-service coverage of at least 1.5 times in the approved base case
- Interest sensitivity at the sanctioned rate and two percentage points higher
- Downside case with sales 25% below target for six months
- Six months of fixed-cost and interest protection
- No brand above one-third of handset capital
- No more than 10% of handset capital above NPR 100,000
- Weekly open-to-buy and stock-ageing approval
- Daily IMEI stock reconciliation
- Separate purchase approval, receiving, payment and reconciliation where practical
- Written authority for discounts, refunds, write-offs and emergency purchases
- No use of debt or BG for a brand until authorised-channel status, warranty route, invoicing, settlement, price protection and stock-rotation exposure are documented

## Expansion and stop rules

Increase inventory utilisation only if:

- Thirty-day sell-through exceeds 70% for the relevant category.
- Total ageing remains below 45 days.
- Effective realised margin meets the approved model.
- Supplier credit notes arrive within the agreed period.
- Cash and debt-service forecasts remain inside limits.

Freeze discretionary purchasing if:

- The store remains below 70 phones per month after month four.
- More than 10% of handset units exceed 45 days.
- Any unexplained IMEI variance remains open.
- A distributor protection term is disputed or not honoured.
- The downside liquidity floor is breached.

## Approval package

Before commitment, attach:

- Bank term sheet and facility schedule
- Distributor term sheets
- Final site lease and rent comparison
- Fit-out and technology quotations
- Approved staffing/payroll
- Opening inventory at cost
- Insurance quotations
- Tax/accounting treatment
- Base, downside and upside cash-flow models
- Founder approval and qualified finance/legal review
